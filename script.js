...
if (document.body.dataset.page === 'profile') {
  ...
  if (player.productImages) {
    ...
    document.querySelector('#gear .gear-grid').innerHTML = products.map(([id, label, name, href]) => `<a class="gear-card product-card" href="${href}" target="_blank" rel="noopener"><span class="product-photo"><img src="${player.productImages[id]}" alt="${name}"></span><small>${label}</small><strong>${name}</strong><span>Ver produto ↗</span></a>`).join('');
  }
